...
export class InitMigration1732429109740 implements MigrationInterface {
    name = 'InitMigration1732429109740'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`role\` (\`id\` int NOT NULL AUTO_INCREMENT, \`name\` enum ('administrator', 'system_coordinator', 'system_assistant', 'system_auxiliary', 'maintenance_coordinator', 'maintenance_auxiliary') NOT NULL, \`description\` text NULL, UNIQUE INDEX \`IDX_ae4578dcaed5adff96595e6166\` (\`name\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
